...
from tkinter import Event, Label, ttk
...
def start_timmer():
    ...
    # button_rest.config(state="disabled")
    for time in range(1,timer_limit+1):
        elpasedTime=time
        lbl_elapsedtimer["text"]=elpasedTime

        updatedremaingtime=reminingTime-elpasedTime
        lbl_reamingtimer["text"]=updatedremaingtime
...
def count():
    ...
    while elpasedTime!=timer_limit:
        entterParagraph=entry.get(1.0,"end-1c").split()
        total_word=len(entterParagraph)

    for pair in (zip(para_words,entterParagraph)):
        if pair[0]!=pair[1]:
    ...
    # wpm
    wpm=(total_word-wrong_words)/elpasedTimeInminute
    lbl_wpm["text"]=wpm
    # accurancy
    gross_wpm=total_word/elpasedTimeInminute
    accuracy=(wpm/gross_wpm)*100
    lbl_accuracy["text"]=round(accuracy,2)
    # total words
    lbl_total_words["text"]=total_word
    lbl_wrong_words["text"]=wrong_words



def start():
    thr1=threading.Thread(target=start_timmer)
    thr1.start()
...
# changing ptagtaph
with open('D:\JIYAN IMP\Typo Speed\github\Typo-Speed---Python-Project\paragraph.txt',encoding="utf8") as f:
    paragraph=f.readlines()
    selected_paragraph=random.choice(paragraph)
...
window.title("typo speed by  ")
window.geometry("1000x950+300+15")
...
lbl_title=tk.Label(title_frame,text="Typo master ",font="algerian 33 bold",bg="yellow",fg="black",relief="flat",bd=10,width=35,justify="center")
lbl_title2=tk.Label(title_frame,text="https://www.linkedin.com/in/jiyan-patil-8a85561a5/",font="algerian 12 bold",bg="yellow",fg="black",relief="flat",bd=10,width=35,justify="center",cursor="hand2")
lbl_title2.bind("<Button-1> ", callback)
lbl_title.grid(row=0,column=0,pady=2)
lbl_title2.grid(row=2,column=0,pady=2)
...
# testFRame
# selected_paragraph="paragraph is a series of sentences that are organized and coherent, and are all related to a single topic. Almost every piece of writing you do that is longer than a few sentences should be organized into paragraphs. ... Paragraphs can contain many different kinds of information."
test_frame=tk.LabelFrame(main_frame,text="test",bg="white",relief="groove",width=10)
    #paragraph
lbl_paragraph=tk.Label(test_frame,text=selected_paragraph,wraplength=1000,justify="left",font=('Helvetica bold',15))
lbl_paragraph.grid(row=0,column=0,pady=5)
test_frame.grid(row=1,column=0)
#     # Inputbox 
entry=tk.Text(test_frame,height=10,width=80,bd=2)
...
# elapse time
lbl_elapsedtime=tk.Label(frame_labels,text="Elapsed time ",font= "Tahoma 10 bold ",fg="red",bg="white")
...
lbl_elapsedtimer.grid(row=0,column=1,pady=10,padx=10)
# remainig time
lbl_reamingtime=tk.Label(frame_labels,text="Remaining time",font= "Tahoma 10 bold ",fg="red",bg="white")
lbl_reamingtimer=tk.Label(frame_labels,text="60",font= "Tahoma 10 bold ",fg="black",bg="white")
lbl_reamingtime.grid(row=0,column=2,padx=10,pady=10)
lbl_reamingtimer.grid(row=0,column=3,pady=10,padx=10)
# wpm
...
lbl_wpm.grid(row=0,column=5,pady=10,padx=10)
# accuracy
...
lbl_accuracy.grid(row=0,column=7,pady=10,padx=10)
# total words
lbl_total_words_title=tk.Label(frame_labels,text="Totalwords",font= "Tahoma 10 bold ",fg="red",bg="white")
...
lbl_total_words.grid(row=0,column=9,pady=10,padx=10)
# wrong words
lbl_wrong_words_title=tk.Label(frame_labels,text="Wrong wordws",font= "Tahoma 10 bold ",fg="red",bg="white")
lbl_wrong_words=tk.Label(frame_labels,text="0",font= "Tahoma, 20",fg="black",bg="white")
lbl_wrong_words_title.grid(row=0,column=10,padx=10,pady=10)
lbl_wrong_words.grid(row=0,column=11,pady=10,padx=10)
...
lbl_space = tk.Label(frame_space,text='',bg='black',fg='white',width=40,height=2,relief='groove',bd=10)
lbl_space.grid(row=0,column=0,padx=0,pady=0)
